...
用法：python scripts/import_cases.py [cases.json]
默认文件：data/seed/cases.json
...
from welding_kg.graph_importer import import_case_json  # noqa: E402
...
def main() -> int:
    ...
    path = (
        sys.argv[1]
        if len(sys.argv) > 1
        else str(Path(__file__).resolve().parents[1] / "data" / "seed" / "cases.json")
    )
    ...
    try:
        result = import_case_json(path, store)
    finally:
        store.close()
